(function( /*CTIOperationRequest*/ request, /*CTIOperationResponse*/ response, /*Context*/ ctx) {
    ...
    try {
        ...
        if ('new' === state) {
			...
			interactionGr.assignment_group.setDisplayValue("ASKIT_Helpdesk_Queue");

            if (foundUser) {
                interactionGr.setValue('opened_for', userGr.getUniqueValue());
                //interactionGr.setValue('opened_for.u_fiuroles', userGr.getUniqueValue());
				...
				response.setSessionAttribute('#', userGr.getDisplayValue('employee_number'));
				response.setSessionAttribute('#', userGr.getDisplayValue('u_fiuroles'));
